...
//  MovieDB
//
//  Created by Dias Shabarov on 15.05.2024.
//
...
class MovieDetailViewController: UIViewController {
    ...
    func apiRequest() {
        NetworkManager.shared.loadMovieDetail(movieId: movieId) { result in
            ...
            if let storage = StorageManager.shared.readData(key: .watchList) as? [Int]
            {
                if let _ = storage.firstIndex(of: self.movieData!.id!) {
    ...
    func setupLayout() {
        ...
        scrollMovieDetail.addSubview(watchListButton)
    ...
    @IBAction
    func addWatchList() {
        guard let movieDataId = movieData?.id else {return}
        if let storage = StorageManager.shared.readData(key: .watchList) as? [Int] {
            if let index = storage.firstIndex(of: movieDataId) {
                StorageManager.shared.removeWatchList(movieDataId, key: .watchList)
                watchListButton.setTitle("Add to Watch List", for: .normal)
                watchListButton.backgroundColor = .systemBlue
            } else
            {
                StorageManager.shared.writeWatchList(movieDataId, key: .watchList)
