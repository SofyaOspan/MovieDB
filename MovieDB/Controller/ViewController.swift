...
//  testNoStoryBoard
//
//  Created by Dias Shabarov on 27.04.2024.
//
import UIKit
import SnapKit
import  CoreData
...
class ViewController: UIViewController {
   ...
   private lazy var movieDBLabel: UILabel = {
    ...
    private lazy var disapearView: UIView = {
        let view = UIView()
        view.alpha = 0
        return view
    }()
   ...
   private func setupUI() {
       ...
       view.addSubview(disapearView)
       [themeLabel,themeCollectionView,tableView].forEach {
           disapearView.addSubview($0)
       }
...
//            make.leading.trailing.equalToSuperview()
            labelXPosition = make.centerX.equalToSuperview().constraint
            labelYPosition = make.centerY.equalToSuperview().constraint
        }
       disapearView.snp.makeConstraints { make in
           make.top.equalTo(movieDBLabel.snp.bottom).offset(20)
   ...
   private func createLayout() -> UICollectionViewLayout {
    ...
    func animate() {
        UIView.animate(withDuration: 1) {
            ...
            UIView.animate(withDuration: 1) {
                ...
                UIView.animate(withDuration: 3, delay: 0.5, usingSpringWithDamping: 0.7, initialSpringVelocity: 0.5, options: .curveEaseOut){
                    self.position()
                } completion: {_ in
                    self.movieDBLabel.alpha = 1
                    self.disapearView.alpha = 1
                }
            }
        }
    ...
    func position() {
        self.labelYPosition.update(offset:  -(view.safeAreaLayoutGuide.layoutFrame.size.height / 2) - 20)
        view.layoutSubviews()
    }
